...
- **挂起保活 (Hang-to-console)**:`tscon <sid> /dest:console`,把 RDP 断开后的会话挂回本机控制台(物理显示器),防止锁屏导致合成输入失效。挂回成功即自动息屏(ADR-0003 语义)。
- **三信号探针 (Probe)**:会话状态(WTS)+ 输入桌面可达性 + 前台窗口。三者合起来才能区分"断开/锁屏/无前台"三种失败模式。
- **无前台 (Null-foreground)**:会话 Active 但 `GetForegroundWindow()==0`——RDP 最小化/过渡态。持续超过**宽限期(90s)**才允许自愈,坚决不踢正在使用的用户。
- **分层自愈 (Layered self-healing, ADR-0006)**:L0 预防(提权+防锁屏)→ L1 巡检(5s 三信号判定)→ L2 发送路径(置前失败即强制自愈+复位窗口+单次重试)→ L3 遥测(审计+邮件告警)。
